package com.kelompok_b.petshop.Api;

public class PetAPI {
    public static final String ROOT_URL   = "https://my-pet-shop.online/";
    public static final String ROOT_API   = ROOT_URL+ "api/";

//    pet api
    public static final String URL_INDEX        = ROOT_API+"pet";
    public static final String URL_SHOW         = ROOT_API+"pet/";
    public static final String URL_SHOW_CAT     = ROOT_API+"petCat";
    public static final String URL_SHOW_DOG     = ROOT_API+"petDog";
    public static final String URL_ADD          = ROOT_API+"pet";
    public static final String URL_UPDATE       = ROOT_API+"pet/";
    public static final String URL_DELETE       = ROOT_API+"pet/";
    public static final String URL_IMAGE        = ROOT_API+"images";
}
